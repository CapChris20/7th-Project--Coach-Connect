...
export default function RoleMigrationScreen({ onClose }) {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setLoading(true);
            try {
              const migrationResult = await migrateUserRoles('client');
  ...
  const handleSetTrainers = async () => {
    ...
    setLoading(true);
    try {
      const trainerResult = await setTrainersByEmail(emails);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Role Migration</Text>
        {onClose && (
      ...
      <ScrollView style={styles.scrollContent}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>1. Set Default Role for All Users</Text>
          ...
          >
            {loading ? (
              ...
              <Text style={styles.buttonText}>Set All Users to Client</Text>
            )}
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>2. Set Specific Users as Trainers</Text>
          <Text style={styles.sectionDescription}>
            Enter email addresses (one per line) of users who should be trainers. These users will be updated to have role: "trainer".
          </Text>
          <Text style={styles.inputLabel}>Trainer Email Addresses:</Text>
          <TextInput
          ...
          >
            {loading ? (
              ...
              <Text style={styles.buttonText}>Set as Trainers</Text>
            )}
          </TouchableOpacity>
        </View>
        ...
        {result && (
          <View style={styles.resultContainer}>
            <Text style={[styles.resultText, { fontWeight: '700', marginBottom: spacing.xs }]}>
              Last Result:
            </Text>
