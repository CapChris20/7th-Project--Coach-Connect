...
function metricProgressPct(key, report) {
  ...
  switch (key) {
    ...
    case 'energy': {
      ...
      return n == null ? 0 : Math.min(n / 5, 1);
    }
    default:
      return 0;
  }
...
function buildMetricTiles(report) {
  ...
  const energy = report?.avgEnergy != null && report.avgEnergy !== 'N/A' ? `${report.avgEnergy}/5` : null;
  const tiles = [
...
export function parseStructuredDayNote(note) {
  ...
  const pillDefs = [
    ...
    },
    {
      key: 'stress',
      label: 'Stress',
      re: /^stress\s+(\d+(?:\.\d+)?)\/10$/i,
      fmt: (m) => `${m[1]}/10`,
    },
    {
      key: 'soreness',
      label: 'Soreness',
      re: /^soreness\s+(\d+(?:\.\d+)?)\/10$/i,
      fmt: (m) => `${m[1]}/10`,
    },
    {
      key: 'weight',
