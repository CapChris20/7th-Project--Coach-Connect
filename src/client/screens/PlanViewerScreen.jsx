...
const RECOVERY_GRADS = [
  ["#9333EA", "#DB2777"],
  ["#7C3AED", "#C084FC"],
  ["#6D28D9", "#A78BFA"],
];

function getRecoveryGrad(index) {
  return RECOVERY_GRADS[Math.abs(Number(index) || 0) % RECOVERY_GRADS.length];
}
  ...
  return (
    <View style={{ borderRadius: radius }}>
      ...
      >
        ...
        >
          <LinearGradient colors={rimColors} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.topStripe} />
          {children}
        </View>
      </LinearGradient>
    </View>
...
function IndexBadge({ index, grad }) {
  return (
    <LinearGradient colors={grad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.indexBadge}>
      ...
      <GradientFrame rimColors={accent.grad} innerBg={innerBg} isDark={isDark}>
        <View style={styles.dayCardInner}>
          <View style={styles.trainHeaderRow}>
            <LinearGradient colors={accent.grad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.dayShortGrad}>
  ...
  return (
    <TouchableOpacity activeOpacity={0.82} onPress={onPress}>
      <GradientFrame rimColors={grad} innerBg={innerBg} isDark={isDark}>
        <View style={styles.dayCardInner}>
          <View style={styles.trainHeaderRow}>
            <LinearGradient colors={grad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.dayShortGrad}>
              <Text style={styles.dayShortText}>{day.short}</Text>
            </LinearGradient>
            <View style={styles.trainTitles}>
...
export default function PlanViewerScreen({ route }) {
  ...
  const recoveryGrad = useMemo(() => getRecoveryGrad(openRecoveryIndex ?? 0), [openRecoveryIndex]);
  ...
  if (openDay) {
    return (
      <View style={{ flex: 1, backgroundColor: bg }}>
        <DayDetailScreen day={openDay} onBack={onBackFromDay} onOpenExercise={(idx) => setExerciseModal(idx)} isDark={isDark} />
        <ExerciseDetailModal
