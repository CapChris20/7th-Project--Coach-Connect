    ...
    try {
      ...
      try {
        ...
        if (userId) {
          ...
          await AsyncStorage.setItem(`onboarding_data_${userId}`, JSON.stringify(updateData)).catch(() => {});
          await queuePendingOnboardingSync(userId, {
            path: '/api/onboarding/complete',
            body: {
              finalRole: chosenRole,
...
• Lifestyle factors we should know`}
  ...
  // Render trainer step — purple/pink Lovable-style cards (matches TrainerOnboardingScreen reference)
