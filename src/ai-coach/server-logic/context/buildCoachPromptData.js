 ...
 */
/**
 * Decides whether the AI Coach should load this user's app history
 * (nutrition, workouts, sleep, etc.) into the system prompt.
 *
 * Why: Loading full account context costs Firestore reads and makes the prompt large.
 ...
 *
 * Data window: since account creation (up to 2 years), with lifetime averages +
 * last 45 days of meal detail + monthly rollups for older months.
 *
 ...
 * @param {Array<{role:string,content?:string,text?:string}>} [messages] — prior turns for thread-aware routing
 * @returns {boolean} true = load account history context; false = skip it
 */
export function shouldIncludeWeeklyContextInCoachPrompt(userText, messages = null) {
  ...
  const personalPatterns = [
    ...
    // food log / nutrition diary access (including pushback on coach denials)
    /\bfood logs?\b/,
    ...
    // all-time / since joining history
...
/** User pushing back after coach wrongly said it can't see their app logs. */
