 ...
 * context Aggregation
 *
 * Purpose: context Aggregation — Feature module for Coach Connect.
 * Why it matters: Keeps feature logic out of screens so auth, nutrition, and trainer rules stay consistent.
 * Area: src/ai
 * Key exports: normalizeCoachContext, fetchWeeklyContextFromServer, aggregateUserContext, loadCoachContext, calculateDataQuality, loadCoachContextEnhanced, buildContextSystemBlock
 *
 * @file-header
 */
/**
...
import { aggregateCoachWeeklyDataClient } from './loadCoachWeeklyStats';
...
export async function aggregateUserContext(userId, userProfile = {}) {
  ...
  try {
    ...
    const [
      ...
      getDoc(doc(db, 'users', userId, 'profile', 'current')),
      getDoc(doc(db, 'users', userId)),
      getDoc(doc(db, 'users', userId, 'macroTargets', 'current')),
      getDoc(doc(db, 'nutrition_goals', userId)),
    ]);

    if (profileCurrentResult.status === 'fulfilled' && profileCurrentResult.value.exists()) {
      profile = { ...profile, ...profileCurrentResult.value.data() };
    }
    if (profileRootResult.status === 'fulfilled' && profileRootResult.value.exists()) {
      profile = { ...profile, ...profileRootResult.value.data() };
    }
    if (macroCurrentResult.status === 'fulfilled' && macroCurrentResult.value.exists()) {
      macroTargets = { ...macroTargets, ...macroCurrentResult.value.data() };
    }
    if (nutritionGoalsResult.status === 'fulfilled' && nutritionGoalsResult.value.exists()) {
      const g = nutritionGoalsResult.value.data() || {};
      macroTargets = {
        ...macroTargets,
    ...
    const [curPlanResult, streakResult, photoResult, notesResult] = await Promise.allSettled([
      getDoc(doc(db, 'users', userId, 'workoutPlans', 'current')),
      getDoc(doc(db, 'users', userId, 'streaks', 'current')),
      getDocs(query(collection(db, 'users', userId, 'progressPhotos'), limit(20))),
      getDocs(query(collection(db, 'users', userId, 'notes_and_files'), limit(8))),
    ]);

    if (curPlanResult.status === 'fulfilled' && curPlanResult.value.exists()) {
      currentPlan = curPlanResult.value.data()?.name || curPlanResult.value.data()?.title || currentPlan;
    } else {
      try {
    ...
    if (streakResult.status === 'fulfilled' && streakResult.value.exists()) {
      streakDays =
        toNum(streakResult.value.data()?.streak) ?? toNum(streakResult.value.data()?.currentStreak) ?? 0;
    }
    if (photoResult.status === 'fulfilled') {
      photoCount = (photoResult.value.docs || []).filter((d) => docInLast7Days(d, startMs)).length;
    }
    if (notesResult.status === 'fulfilled') {
      const lines = (notesResult.value.docs || [])
...
async function runMacroRecalibrationViaServer(userId) {
  ...
  const { shouldRecalibrateMacros } = await import('../macro-recalibration/recalculateMacrosFromCoach');
