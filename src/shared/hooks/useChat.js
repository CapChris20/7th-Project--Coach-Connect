...
import { sendChatMessage, generateWorkoutResponse } from '../../ai/services/chatService';
import { saveApiKey, hasApiKey } from '../../ai/services/apiKeyService';
import {
  getChatById,
  saveChat,
  updateChatMessages,
  setCurrentChatId,
  createNewChat,
} from '../../ai/services/chatStorageService';
...
export function useChat(chatId = null) {
  ...
  const sendMessage = useCallback(async (message, apiKey = null, imageData = null) => {
    ...
    // Detect if query needs web search indicator (web search is always enabled by default)
    // Show "Currently on the web" indicator when keywords suggest current info is needed
    const needsWebSearch = detectWebSearchNeeded(userMessage);
    setIsSearchingWeb(needsWebSearch);
  ...
  /**
   * Detect if a query needs web search based on keywords
   * Always returns true - web search is always enabled, but this helps show the indicator
   */
  function detectWebSearchNeeded(query) {
    if (!query || typeof query !== 'string') return true; // Default to web search enabled
    
    const lowerQuery = query.toLowerCase();
    
    // Keywords that strongly suggest current/recent information is needed
    const webSearchKeywords = [
      'latest', 'current', 'recent', 'new', 'news', 'today', 'now', 'this week', 'this month',
      '2024', '2025', 'update', 'trend', 'trending', 'popular', 'best', 'top',
      'what is', 'who is', 'when did', 'where is', 'how much', 'how many',
      'research', 'study', 'study shows', 'according to', 'source', 'citation'
    ];
    
    // If it matches keywords, definitely show web search indicator
    // Otherwise, still enable web search but don't show the indicator
