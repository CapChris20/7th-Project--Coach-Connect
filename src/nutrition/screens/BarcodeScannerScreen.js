...
export default function BarcodeScannerScreen({ onClose, onScanSuccess, mealType }) {
  ...
  const lookupBarcode = async (barcode) => {
    ...
    try {
      ...
      const raw = await foodSearchProvider.lookupBarcode(barcode.trim());
      const result = raw
      ...
      if (result.success && result.data) {
        ...
        Alert.alert(
          'Product Not Found',
          result.error || 'This barcode was not found in our database. Please try searching by name instead.',
          [
