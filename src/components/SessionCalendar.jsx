    ...
    <View style={[styles.card, { backgroundColor: colors.glass, borderColor: colors.border }]}>
      ...
      {weeks.map((row, rIdx) => (
        <View key={rIdx} style={styles.row}>
          {row.map((cell, cIdx) => {
            if (!cell) return <View key={cIdx} style={styles.cell} />;
            ...
            return (
              ...
              >
                ...
                {hasSession && <View style={[styles.dot, { backgroundColor: isSelected ? 'white' : colors.primary }]} />}
              </TouchableOpacity>
            );
          })}
        </View>
...
const styles = StyleSheet.create({
  ...
  weekdays: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6, paddingHorizontal: 6 },
