...
import { buildContextSystemBlock } from './contextAggregation';
import { normalizeToolCall } from './toolExecutor';
import {
  parseCoachToolCalls,
  stripCoachToolJsonFromReply,
} from '../shared/parseCoachToolCalls';
import { shouldIncludeWeeklyContextInCoachPrompt } from './coachPersonalDataRouting';
...
async function postAICoach(payload, options = {}) {
  ...
  for (const base of bases) {
    ...
    try {
      ...
      if (__DEV__) console.debug('✅ AI Coach response (/api/ai-coach)', url);
...
export { loadCoachContextEnhanced, calculateDataQuality } from './contextAggregation';
